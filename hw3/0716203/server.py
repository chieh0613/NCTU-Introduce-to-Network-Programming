...
chatroom_list = [] # 0: ip, 1:port, 2:status, 3: chatroom_name
...
def udp_job():
    while True:
        ...
        if command[0]=='register':
            ...
            result=whoami(id)
        elif command[0] == 'list-chatroom':
            pos=indata.decode().index("&")
            id=int(indata.decode()[pos+1:])
            result=list_chatroom(command,id)
...
def TCP_socket():
    while True:
        conn, client_addr = tcp_s.accept()
        print("New connection.")
        tcp_thread=threading.Thread(target=tcp_job,args=(conn,client_addr[0], ))
        tcp_thread.start()
...
def logout(status,id,author):
    if status==0:
        ...
        while(idx<len(chatroom_list)):
            if user_name in chatroom_list[idx] and chatroom_list[idx][2] == 'open':
                return 'Please do “attach” and “leave-chatroom” first', status, author
            idx+= 1
        ...
        status=0
        author="logout"
        return "Bye, " + user_name, status, author
...
def create_chatroom(indata,status,id,client_ip):
    #print(conn)
    if len(indata)<2:
        ...
        tmp=c.execute("SELECT username from list where UID==(?)",(id,))
        user=tmp.fetchall()
        chatroom_name = user[0][0]
        idx = 0
        created = 0
        while(idx < len(chatroom_list)):
            if(chatroom_name in chatroom_list[idx]):
                created = 1
                break
            idx += 1
        ...
        if created == 1:
            ...
            # chatroom_list    0: ip, 1:port, 2:status, 3: chatroom_name
            chatroom_list.append([client_ip,indata[1],'open',chatroom_name])
            #print(chatroom_list)       
            return 'start to create chatroom...'+'&'+client_ip+'&'+str(indata[1])

        
def list_chatroom(indata,id):
    if len(indata)!=3: ### list-chatroom, &, id => length = 3
        ...
        if cnt[0][0]==0:
            ...
            str1 = 'Chatroom_name'+'\t'+ 'Status'
            idx = 0
            while(idx < len(chatroom_list)):
                # chatroom_list    0: ip, 1:port, 2:status, 3: chatroom_name
                #print(type(chatroom_list[idx][3],'  ',chatroom_list[idx][3] ))
                
                str1 +='\n'+ chatroom_list[idx][3] + '\t\t ' + chatroom_list[idx][2]
                idx += 1
                    
            return str1
                

def join_chatroom(indata,status,client_ip):      
    if len(indata)!=2: 
        ...
        idx = 0
        chatroom_name = indata[1]
        port = -1
        while(idx<len(chatroom_list)):
            if chatroom_name in chatroom_list[idx] and chatroom_list[idx][2] == 'open':
                port = chatroom_list[idx][1]
                break
            elif chatroom_name in chatroom_list[idx] and chatroom_list[idx][2] == 'close':
        ...
        if port == -1:
            ...
            return 'connection to chatroom server' + '&' + client_ip +'&' +str(port)


def leave_chatroom(indata):
    ...
    while(idx < len(chatroom_list)):
        if (chatroom_name in chatroom_list[idx]):
            break
        idx += 1
    chatroom_list[idx][2] = 'close'
    

def restart_chatroom(indata, status, id):
    if len(indata)!=1: 
        ...
        if found == -1:
            ...
            chatroom_list[idx][2] = 'open'
            return 'restart chatroom server'
...
def tcp_job(conn, client_ip):
    ...
    while True:
        ...
        if command[0]=='login':
            ...
            conn.sendall(result.encode())
        elif command[0] == 'create-chatroom':
            result = create_chatroom(command,status,id,client_ip)
            conn.sendall(result.encode())
        elif command[0] == 'join-chatroom':
            result=join_chatroom(command,status,client_ip)
            conn.sendall(result.encode())
        elif command[0] == 'leave-chatroom':
            leave_chatroom(command)
        elif command[0] == 'restart-chatroom':
            result=restart_chatroom(command,status,id)
            conn.sendall(result.encode())
